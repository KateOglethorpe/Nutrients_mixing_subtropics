...
S_file = 'woa18_decav_s00_01.nc';            %salinity 
...
%N, T, S, w_mld dimensions
lon_woa = ncread(T_file, 'lon');             
lat_woa = ncread(T_file, 'lat');                     
z_woa = ncread(T_file, 'depth'); 
%K_iso dimensions
lon_Groeskamp = WOA18_K.x;                 
...
chl = load('Chl_Feb_Aqua_Modis.txt')
chl(chl == 99999) = NaN;
chl = flipud(chl);
...
T_z = permute(T_z, [2,1,3]); %[degC]               	 
S_z = permute(S_z, [2,1,3]);                     
N_z = permute(N_z, [2,1,3]); %[micromol kg-1]
w_mld_z = permute(w_mld_z, [2, 1]); %[m]
K_iso_z = permute(K_iso_z, [2,1,3]); %[m2 s-1]
...
K_iso_new = [K_iso_1st K_iso_2nd]; %[m2 s-1]
...
K_dia_new = [K_dia_1st K_dia_2nd]; %[m2 s-1]

%map K_dia on 1 deg res
K_dia_1deg = interp3(lon_new, lat_Whalen,z_Whalen, K_dia_new, ...
    double(lon_woa'), double(lat_woa),z_Whalen); %[m2 s-1]
...
S_z = S_z(:,:,1:47);       
T_z = T_z(:,:, 1:47);
N_z = N_z(:,:, 1:47); %[micromol kg-1]
K_iso_z = K_iso_new(:,:, 1:47); %[m2 s-1]
K_dia_z = K_dia_1deg(:,:, 1:4); %[m2 s-1]
...
m_pcolor(glob_lon, glob_lat, K_dia_z(:,:,1))
hold on
m_grid('fontsize',9);
m_coast('color','k')
set(gcf,'color','w')
...
set(gca,'Fontsize',9)
title('Diapycnal diffusivity at 300m')

%% Convert N conc (micromol m-3) to N (mol m-3)
...
[gamma_n_z] = eos80_legacy_gamma_n(S_z,T_z,p_z,glob_lon,glob_lat); % [kg m^-3]
%save('gamma_n_z.mat','gamma_n_z');
%load gamma_n_z.mat

density = gamma_n_z + 1000;  %[kg m^-3]        
nitrate_z= N_z.* density; 
nitrate_z= nitrate_z./ 1e6; 
save('nitrate_z', 'nitrate_z')
...
%% Quantify uncertainty by changing the gyre masks

% Generate expanded and shrunken masks (~20% greater and smaller area)

% Get the size of the mask
[rows, cols] = size(NA_mask);

%%%%%%North Atlantic%%%%%%%%

expanded_NA_mask = NA_mask;
% Loop through each cell in the mask
for i = 2:rows-1
    for j = 2:cols-1
        % Check if the current cell is 1
        if NA_mask(i, j) == 1
            % Set surrounding cells to 1
            expanded_NA_mask(i-1, j) = 1; % Top
            expanded_NA_mask(i+1, j) = 1; % Bottom
            expanded_NA_mask(i, j-1) = 1; % Left
            expanded_NA_mask(i, j+1) = 1; % Right
            expanded_NA_mask(i-1, j-1) = 1; % Top-left
            expanded_NA_mask(i-1, j+1) = 1; % Top-right
            expanded_NA_mask(i+1, j-1) = 1; % Bottom-left
            expanded_NA_mask(i+1, j+1) = 1; % Bottom-right
        end
    end
end

shrunken_NA_mask = NA_mask;
% Loop through each cell in the mask
for i = 2:rows-1
    for j = 2:cols-1
        % Check if the current cell is 1
        if NA_mask(i, j) == 1
            % Check if any surrounding cells are 0
            if NA_mask(i-1, j) == 0 || ... % Top
               NA_mask(i+1, j) == 0 || ... % Bottom
               NA_mask(i, j-1) == 0 || ... % Left
               NA_mask(i, j+1) == 0 || ... % Right
               NA_mask(i-1, j-1) == 0 || ... % Top-left
               NA_mask(i-1, j+1) == 0 || ... % Top-right
               NA_mask(i+1, j-1) == 0 || ... % Bottom-left
               NA_mask(i+1, j+1) == 0       % Bottom-right
           
                % Set the current cell to 0
                shrunken_NA_mask(i, j) = 0;
            end
        end
    end
end
...
for i = 2:rows-1
    for j = 2:cols-1
        ...
        if SA_mask(i, j) == 1
            % Set surrounding cells to 1
            expanded_SA_mask(i-1, j) = 1; % Top
            expanded_SA_mask(i+1, j) = 1; % Bottom
            expanded_SA_mask(i, j-1) = 1; % Left
            expanded_SA_mask(i, j+1) = 1; % Right
            expanded_SA_mask(i-1, j-1) = 1; % Top-left
            expanded_SA_mask(i-1, j+1) = 1; % Top-right
            expanded_SA_mask(i+1, j-1) = 1; % Bottom-left
            expanded_SA_mask(i+1, j+1) = 1; % Bottom-right
        end
    end
end

shrunken_SA_mask = SA_mask;
% Loop through each cell in the mask
for i = 2:rows-1
    for j = 2:cols-1
        % Check if the current cell is 1
        if SA_mask(i, j) == 1
            % Check if any surrounding cells are 0
            if SA_mask(i-1, j) == 0 || ... % Top
               SA_mask(i+1, j) == 0 || ... % Bottom
               SA_mask(i, j-1) == 0 || ... % Left
               SA_mask(i, j+1) == 0 || ... % Right
               SA_mask(i-1, j-1) == 0 || ... % Top-left
               SA_mask(i-1, j+1) == 0 || ... % Top-right
               SA_mask(i+1, j-1) == 0 || ... % Bottom-left
               SA_mask(i+1, j+1) == 0       % Bottom-right
           
                % Set the current cell to 0
                shrunken_SA_mask(i, j) = 0;
            end
        end
    end
end
...
for i = 2:rows-1
    for j = 2:cols-1
        ...
        if I_mask(i, j) == 1
            % Set surrounding cells to 1
            expanded_I_mask(i-1, j) = 1; % Top
            expanded_I_mask(i+1, j) = 1; % Bottom
            expanded_I_mask(i, j-1) = 1; % Left
            expanded_I_mask(i, j+1) = 1; % Right
            expanded_I_mask(i-1, j-1) = 1; % Top-left
            expanded_I_mask(i-1, j+1) = 1; % Top-right
            expanded_I_mask(i+1, j-1) = 1; % Bottom-left
            expanded_I_mask(i+1, j+1) = 1; % Bottom-right
        end
    end
end

shrunken_I_mask = I_mask;
% Loop through each cell in the mask
for i = 2:rows-1
    for j = 2:cols-1
        % Check if the current cell is 1
        if I_mask(i, j) == 1
            % Check if any surrounding cells are 0
            if I_mask(i-1, j) == 0 || ... % Top
               I_mask(i+1, j) == 0 || ... % Bottom
               I_mask(i, j-1) == 0 || ... % Left
               I_mask(i, j+1) == 0 || ... % Right
               I_mask(i-1, j-1) == 0 || ... % Top-left
               I_mask(i-1, j+1) == 0 || ... % Top-right
               I_mask(i+1, j-1) == 0 || ... % Bottom-left
               I_mask(i+1, j+1) == 0       % Bottom-right
           
                % Set the current cell to 0
                shrunken_I_mask(i, j) = 0;
            end
        end
    end
end
...
for i = 2:rows-1
    for j = 2:cols-1
        ...
        if NP1_mask(i, j) == 1
            % Set surrounding cells to 1
            expanded_NP1_mask(i-1, j) = 1; % Top
            expanded_NP1_mask(i+1, j) = 1; % Bottom
            expanded_NP1_mask(i, j-1) = 1; % Left
            expanded_NP1_mask(i, j+1) = 1; % Right
            expanded_NP1_mask(i-1, j-1) = 1; % Top-left
            expanded_NP1_mask(i-1, j+1) = 1; % Top-right
            expanded_NP1_mask(i+1, j-1) = 1; % Bottom-left
            expanded_NP1_mask(i+1, j+1) = 1; % Bottom-right
        end
    end
end

shrunken_NP1_mask = NP1_mask;
% Loop through each cell in the mask
for i = 2:rows-1
    for j = 2:cols-1
        % Check if the current cell is 1
        if NP1_mask(i, j) == 1
            % Check if any surrounding cells are 0
            if NP1_mask(i-1, j) == 0 || ... % Top
               NP1_mask(i+1, j) == 0 || ... % Bottom
               NP1_mask(i, j-1) == 0 || ... % Left
               NP1_mask(i, j+1) == 0 || ... % Right
               NP1_mask(i-1, j-1) == 0 || ... % Top-left
               NP1_mask(i-1, j+1) == 0 || ... % Top-right
               NP1_mask(i+1, j-1) == 0 || ... % Bottom-left
               NP1_mask(i+1, j+1) == 0       % Bottom-right
           
                % Set the current cell to 0
                shrunken_NP1_mask(i, j) = 0;
            end
        end
    end
end

expanded_NP2_mask = NP2_mask;
% Loop through each cell in the mask
for i = 2:rows-1
    for j = 2:cols-1
        % Check if the current cell is 1
        if NP2_mask(i, j) == 1
            % Set surrounding cells to 1
            expanded_NP2_mask(i-1, j) = 1; % Top
            expanded_NP2_mask(i+1, j) = 1; % Bottom
            expanded_NP2_mask(i, j-1) = 1; % Left
            expanded_NP2_mask(i, j+1) = 1; % Right
            expanded_NP2_mask(i-1, j-1) = 1; % Top-left
            expanded_NP2_mask(i-1, j+1) = 1; % Top-right
            expanded_NP2_mask(i+1, j-1) = 1; % Bottom-left
            expanded_NP2_mask(i+1, j+1) = 1; % Bottom-right
        end
    end
end

shrunken_NP2_mask = NP2_mask;
% Loop through each cell in the mask
for i = 2:rows-1
    for j = 2:cols-1
        % Check if the current cell is 1
        if NP2_mask(i, j) == 1
            % Check if any surrounding cells are 0
            if NP2_mask(i-1, j) == 0 || ... % Top
               NP2_mask(i+1, j) == 0 || ... % Bottom
               NP2_mask(i, j-1) == 0 || ... % Left
               NP2_mask(i, j+1) == 0 || ... % Right
               NP2_mask(i-1, j-1) == 0 || ... % Top-left
               NP2_mask(i-1, j+1) == 0 || ... % Top-right
               NP2_mask(i+1, j-1) == 0 || ... % Bottom-left
               NP2_mask(i+1, j+1) == 0       % Bottom-right
           
                % Set the current cell to 0
                shrunken_NP2_mask(i, j) = 0;
            end
        end
    end
end
...
for i = 2:rows-1
    for j = 2:cols-1
        ...
        if SP1_mask(i, j) == 1
            % Set surrounding cells to 1
            expanded_SP1_mask(i-1, j) = 1; % Top
            expanded_SP1_mask(i+1, j) = 1; % Bottom
            expanded_SP1_mask(i, j-1) = 1; % Left
            expanded_SP1_mask(i, j+1) = 1; % Right
            expanded_SP1_mask(i-1, j-1) = 1; % Top-left
            expanded_SP1_mask(i-1, j+1) = 1; % Top-right
            expanded_SP1_mask(i+1, j-1) = 1; % Bottom-left
            expanded_SP1_mask(i+1, j+1) = 1; % Bottom-right
        end
    end
end

shrunken_SP1_mask = SP1_mask;
% Loop through each cell in the mask
for i = 2:rows-1
    for j = 2:cols-1
        % Check if the current cell is 1
        if SP1_mask(i, j) == 1
            % Check if any surrounding cells are 0
            if SP1_mask(i-1, j) == 0 || ... % Top
               SP1_mask(i+1, j) == 0 || ... % Bottom
               SP1_mask(i, j-1) == 0 || ... % Left
               SP1_mask(i, j+1) == 0 || ... % Right
               SP1_mask(i-1, j-1) == 0 || ... % Top-left
               SP1_mask(i-1, j+1) == 0 || ... % Top-right
               SP1_mask(i+1, j-1) == 0 || ... % Bottom-left
               SP1_mask(i+1, j+1) == 0       % Bottom-right
           
                % Set the current cell to 0
                shrunken_SP1_mask(i, j) = 0;
            end
        end
    end
end

expanded_SP2_mask = SP2_mask;
% Loop through each cell in the mask
for i = 2:rows-1
    for j = 2:cols-1
        % Check if the current cell is 1
        if SP2_mask(i, j) == 1
            % Set surrounding cells to 1
            expanded_SP2_mask(i-1, j) = 1; % Top
            expanded_SP2_mask(i+1, j) = 1; % Bottom
            expanded_SP2_mask(i, j-1) = 1; % Left
            expanded_SP2_mask(i, j+1) = 1; % Right
            expanded_SP2_mask(i-1, j-1) = 1; % Top-left
            expanded_SP2_mask(i-1, j+1) = 1; % Top-right
            expanded_SP2_mask(i+1, j-1) = 1; % Bottom-left
            expanded_SP2_mask(i+1, j+1) = 1; % Bottom-right
        end
    end
end

shrunken_SP2_mask = SP2_mask;
% Loop through each cell in the mask
for i = 2:rows-1
    for j = 2:cols-1
        % Check if the current cell is 1
        if SP2_mask(i, j) == 1
            % Check if any surrounding cells are 0
            if SP2_mask(i-1, j) == 0 || ... % Top
               SP2_mask(i+1, j) == 0 || ... % Bottom
               SP2_mask(i, j-1) == 0 || ... % Left
               SP2_mask(i, j+1) == 0 || ... % Right
               SP2_mask(i-1, j-1) == 0 || ... % Top-left
               SP2_mask(i-1, j+1) == 0 || ... % Top-right
               SP2_mask(i+1, j-1) == 0 || ... % Bottom-left
               SP2_mask(i+1, j+1) == 0       % Bottom-right
           
                % Set the current cell to 0
                shrunken_SP2_mask(i, j) = 0;
            end
        end
    end
end

%% Calculate areas of the expanded and shrunken masks - it's ~20% greater and smaller

% Calculate area 1 x 1 degree
...
%North Atlantic
expanded_mask_area = sum(sum(expanded_NA_mask.*area, 'omitnan'),'omitnan') 
normal_mask_area = sum(sum(NA_mask.*area, 'omitnan'),'omitnan')
shrunken_mask_area = sum(sum(shrunken_NA_mask.*area, 'omitnan'),'omitnan') 
shrunken_mask_area/normal_mask_area %19% smaller
expanded_mask_area/normal_mask_area %19% larger

%South Atlantic
expanded_mask_area = sum(sum(expanded_SA_mask.*area, 'omitnan'),'omitnan') 
normal_mask_area = sum(sum(SA_mask.*area, 'omitnan'),'omitnan')
shrunken_mask_area = sum(sum(shrunken_SA_mask.*area, 'omitnan'),'omitnan') 
shrunken_mask_area/normal_mask_area %16% smaller
expanded_mask_area/normal_mask_area %17% larger

%Indian Ocean
expanded_mask_area = sum(sum(expanded_I_mask.*area, 'omitnan'),'omitnan') 
normal_mask_area = sum(sum(I_mask.*area, 'omitnan'),'omitnan')
shrunken_mask_area = sum(sum(shrunken_I_mask.*area, 'omitnan'),'omitnan') 
shrunken_mask_area/normal_mask_area %17% smaller
expanded_mask_area/normal_mask_area %18% larger

%North Pacific
expanded_mask_area = sum(sum(expanded_NP1_mask.*area, 'omitnan'),'omitnan') + sum(sum(expanded_NP2_mask.*area, 'omitnan'),'omitnan')
normal_mask_area = sum(sum(NP1_mask.*area, 'omitnan'),'omitnan') + sum(sum(NP2_mask.*area, 'omitnan'),'omitnan')
shrunken_mask_area = sum(sum(shrunken_NP1_mask.*area, 'omitnan'),'omitnan') + sum(sum(shrunken_NP2_mask.*area, 'omitnan'),'omitnan') 
shrunken_mask_area/normal_mask_area %12% smaller
expanded_mask_area/normal_mask_area %13% larger

%South Pacific
expanded_mask_area = sum(sum(expanded_SP1_mask.*area, 'omitnan'),'omitnan') + sum(sum(expanded_SP2_mask.*area, 'omitnan'),'omitnan')
normal_mask_area = sum(sum(SP1_mask.*area, 'omitnan'),'omitnan') + sum(sum(SP2_mask.*area, 'omitnan'),'omitnan')
shrunken_mask_area = sum(sum(shrunken_SP1_mask.*area, 'omitnan'),'omitnan') + sum(sum(shrunken_SP2_mask.*area, 'omitnan'),'omitnan') 
shrunken_mask_area/normal_mask_area %15% smaller
expanded_mask_area/normal_mask_area %14% larger
